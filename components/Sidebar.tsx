...
export default function Sidebar() {
    ...
    return (
        <nav className="w-[15%] flex flex-col p-4 dark:bg-custom_card_bg h-max_height rounded-md dark:border-none border-[1px]">
            {
                navList.map((item) => {
                    return <Link key={item?.id} className={`w-100 flex items-center p-2 mb-2 dark:bg-black rounded-md hover:scale-105 duration-300 hover:shadow-custom_orange shadow-sm ${pathname === item?.link && "shadow-custom_orange shadow-sm text-custom_orange"}`} href={item?.link!}>{item?.icon} {item?.title}</Link>
                })
            }
        </nav>
    )
